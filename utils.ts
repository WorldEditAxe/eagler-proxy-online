...
import { ChannelMessageType, Chat, ChatColor, ProxiedPlayer, ProxyClientOptions, State, UUID } from "./types.js"
...
import { deleteBan, fetchBadLoginAttempts, fetchBan, fetchTokenForUser } from "./db.js"
import { joinServerEventBus } from "./serverIPC.js"
import { bungeeJump, gatherJumpData } from "./authServer/bungee.js"
...
export function disconnect(player: ProxiedPlayer, message: Chat | string, code?: DisconnectReason) {
    if (player.state != State.PRE_HANDSHAKE) {
...
export function connectServerAuth(client: ProxiedPlayer) {
    return new Promise<void>(async (res, rej) => {
        ...
        mcClient.on('connect', () => {
            ...
            logger.info(`Player ${client.username} has been connected to the integrated authentication server.`)
            res()
        })
        mcClient.on('raw', p => {
...
export async function doHandshake(client: ProxiedPlayer, initialPacket: Buffer) {
    ...
    const banData = await fetchBan(client.ip)
    if (banData) {
        if (banData.bannedTill < Date.now()) await deleteBan(banData.ip)
        else return disconnect(client, banData.reason)
    }
    client.ws.on('close', () => {
    ...
    try { client.loginOptions = parseUrl(client.url) }
    catch (err) {
        disconnect(client, err.message)
        return
    }
    const identifyC = {
        id: null,
    ...
    if (login[0] === EaglerPacketId.LOGIN) {
        ...
        if (true) {
            ...
            if (true) {
                ...
                client.state = State.AUTHING

                logger.info(`Client [/${client.ip}:${client.remotePort}] as player ${client.username} (${client.uuid}) has been connected to the authentication server.`)

                try { await connectServerAuth(client) }
                catch (err) {
                    logger.error(`Could not connect to internal authentication server: ${err}`)
...
export function parseUrl(url: string): ProxyClientOptions {
    const parsed = parse(url, true), retObj: ProxyClientOptions = {
        server: null
    }
    if (!parsed.query)
        throw new Error("No query passed in WebSocket connection options")
    retObj.server = parsed.query.server as string
    if (!retObj.server) throw new Error("Query object is missing server address")
    return retObj
}