...
let global_verbose: boolean = process.argv.includes("--verbose") ? true : false
...
export class Logger {
    ...
    warn(s: string) {
        if (!this.jsonLog) process.stderr.write(`${color.yellow("W")} ${color.gray(new Date().toISOString())} ${color.yellow(`${color.yellow(`${this.loggerName}:`)} ${s}`)}\n`)
        else (process.stderr as any).write(jsonLog("warn", s))
    }
    ...
    error(s: string) {
        ...
        else (process.stderr as any).write(jsonLog("error", s))
    }
    ...
    fatal(s: string) {
        ...
        else (process.stderr as any).write(jsonLog("fatal", s))
    }
